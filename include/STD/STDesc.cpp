#include "STDesc.h"
void down_sampling_voxel(PointCloud& pl_feat, double voxel_size) {
  if (voxel_size < 0.01) {
    return;
  }
  ...
  for (uint i = 0; i < plsize; i++) {
    ...
    if (iter != voxel_map.end()) {
      ...
      iter->second.count++;
      if (p_c.r > 0 && p_c.g > 0 && p_c.b > 0) {
        iter->second.rgb[0] += p_c.r;
        iter->second.rgb[1] += p_c.g;
        iter->second.rgb[2] += p_c.b;
        iter->second.color_count++;
      }
    } else {
      M_POINT anp;
      anp.xyz[0] = p_c.x;
  ...
  for (auto iter = voxel_map.begin(); iter != voxel_map.end(); ++iter) {
    ...
    pl_feat[i].r = uint8_t(iter->second.rgb[0] / iter->second.color_count);
    pl_feat[i].g = uint8_t(iter->second.rgb[1] / iter->second.color_count);
    pl_feat[i].b = uint8_t(iter->second.rgb[2] / iter->second.color_count);
    i++;
  }
}
...
void read_parameters(ros::NodeHandle& nh, ConfigSetting& config_setting) {
  ...
  // candidate search
  nh.param<int>("loop/skip_near_num", config_setting.skip_near_num_, 10);
  nh.param<int>("loop/candidate_num", config_setting.candidate_num_, 50);
  nh.param<double>("loop/cdesc_diff_thresh", config_setting.cdesc_diff_thresh_, 0.5);
  nh.param<bool>("loop/cdesc_verify_en", config_setting.cdesc_verify_en_, false);
  nh.param<double>("loop/side_length_thresh", config_setting.side_length_thresh_, 0.01);
  nh.param<double>("loop/vertex_diff_thresh", config_setting.vertex_diff_thresh_, 0.5);
  nh.param<double>("loop/planegeo_icp_thresh", config_setting.planegeo_icp_thresh_, 0.4);
  nh.param<double>("loop/planegeo_normal_thresh", config_setting.planegeo_normal_thresh_, 0.2);
  nh.param<double>("loop/planegeo_dis_thresh", config_setting.planegeo_dis_thresh_, 0.5);
  ...
  std::cout << "loop detection threshold: " << config_setting.planegeo_icp_thresh_ << std::endl;
...
void publish_std_descs(
    const std::vector<STDesc>& std_pairs,
  ...
  for (auto desc : std_pairs) {
    ...
    m_line.color.a = 0.8;
    m_line.points.clear();
    m_line.color.r = 52.0 / 255;
    m_line.color.g = 152.0 / 255;
    m_line.color.b = 219.0 / 255;
    geometry_msgs::Point p;
    p.x = desc.vertex_A_[0];
    p.y = desc.vertex_A_[1];
    p.z = desc.vertex_A_[2];
    Eigen::Vector3d t_p;
    t_p << p.x, p.y, p.z;
    p.x = t_p[0];
    p.y = t_p[1];
    ...
    p.x = desc.vertex_B_[0];
    p.y = desc.vertex_B_[1];
    p.z = desc.vertex_B_[2];
    t_p << p.x, p.y, p.z;
    p.x = t_p[0];
    p.y = t_p[1];
    p.z = t_p[2];
    ...
    m_line.id++;
    m_line.points.clear();
    p.x = desc.vertex_C_[0];
    p.y = desc.vertex_C_[1];
    p.z = desc.vertex_C_[2];
    t_p << p.x, p.y, p.z;
    p.x = t_p[0];
    p.y = t_p[1];
    p.z = t_p[2];
...
void STDescManager::GenerateSTDescs(
  ...
  pcl::PointCloud<pcl::PointXYZINormal>::Ptr corner_points(
      new pcl::PointCloud<pcl::PointXYZINormal>);
  std::vector<Eigen::Matrix64f> corner_cdescs;
  corner_extractor(voxel_map, input_cloud, corner_points, corner_cdescs);
  corner_cloud_vec_.push_back(corner_points);

  // step4, generate stable triangle descriptors
  stds_vec.clear();
  build_stdesc(corner_points, corner_cdescs, stds_vec);
...
void STDescManager::SearchLoop(
  ...
  loop_scores.clear();
  best_std_pair.clear();
  if (stds_vec.size() == 0) return;

  std::vector<STDMatchList> candidate_matcher_vec;
  candidate_descriptor_selector(stds_vec, candidate_matcher_vec);

  int total_candidates = candidate_matcher_vec.size();
  loop_candidates.reserve(total_candidates);
  loop_scores.reserve(total_candidates);
  ...
  for (size_t i = 0; i < candidate_matcher_vec.size(); i++) {
    ...
    if (verify_score > best_score) {
      ...
      best_std_pair = sucess_match_vec;
    }
    if (verify_score > 0.) {
      loop_candidates.emplace_back(candidate_matcher_vec[i].match_id_.second);
      loop_scores.emplace_back(verify_score);
    }
  }
  ...
  if (!loop_candidates.empty()) {
    std::vector<size_t> indices(loop_candidates.size());
    std::iota(indices.begin(), indices.end(), 0);
    std::sort(indices.begin(), indices.end(),
              [&](size_t i, size_t j) { return loop_scores[i] > loop_scores[j]; });

    std::vector<int> sorted_candidates;
    std::vector<double> sorted_scores;
    sorted_candidates.reserve(loop_candidates.size());
    sorted_scores.reserve(loop_scores.size());
    for (size_t idx : indices) {
      sorted_candidates.push_back(loop_candidates[idx]);
      sorted_scores.push_back(loop_scores[idx]);
    }
    loop_candidates = std::move(sorted_candidates);
    loop_scores = std::move(sorted_scores);
    best_candidate_id = loop_candidates[0];
    best_candidate_score = loop_scores[0];
  }
}

void STDescManager::AddSTDescs(const std::vector<STDesc>& stds_vec, const Eigen::Affine3d& pose) {
  current_frame_id_++;
  key_pose_vec_.push_back(pose);
  ...
  ROS_INFO("[Loop Detection] Added frame %u with %zu descriptors, total frames: %zu",
           current_frame_id_, stds_vec.size(), key_pose_vec_.size());
  return;
}
...
void STDescManager::corner_extractor(
  ...
  pcl::PointCloud<pcl::PointXYZINormal>::Ptr prepare_corner_points(
      new pcl::PointCloud<pcl::PointXYZINormal>);
  std::vector<Eigen::Matrix64f> prepare_corner_cdescs;
      ...
      for (int i = 0; i < 6; i++) {
        if (current_octo->connect_[i]) {
          ...
          if (current_octo->voxel_points_.size() > 10) {
            ...
            std::vector<Eigen::Vector6d> proj_points;
              ...
              if (iter_near != voxel_map.end()) {
                ...
                if (!voxel_map[connect_project_position]->plane_ptr_->is_plane_) {
                  ...
                  for (size_t j = 0; j < voxel_map[connect_project_position]->voxel_points_.size(); j++) {
                    Eigen::Vector3d pt = voxel_map[connect_project_position]->voxel_points_[j];
                    Eigen::Matrix<double, 6, 1> pt6;
                    pt6 << pt[0], pt[1], pt[2], 0, 0, 0;
                    for (size_t k = 0; k < input_cloud->size(); k++) {
                      const auto& rgb_pt = input_cloud->points[k];
                      if (fabs(rgb_pt.x - pt[0]) < 1e-3 && fabs(rgb_pt.y - pt[1]) < 1e-3 && fabs(rgb_pt.z - pt[2]) < 1e-3) {
                        pt6[3] = rgb_pt.r;
                        pt6[4] = rgb_pt.g;
                        pt6[5] = rgb_pt.b;
                        break;
            ...
            pcl::PointCloud<pcl::PointXYZINormal>::Ptr sub_corner_points(
                new pcl::PointCloud<pcl::PointXYZINormal>);
            std::vector<Eigen::Matrix64f> sub_corner_cdescs;
            extract_corner(projection_center, projection_normal, proj_points,
                           sub_corner_points, sub_corner_cdescs);
            assert(sub_corner_points->size() == sub_corner_cdescs.size());
            for (size_t i = 0; i < sub_corner_points->size(); i++) {
              prepare_corner_points->push_back(sub_corner_points->points[i]);
              prepare_corner_cdescs.push_back(sub_corner_cdescs[i]);
            }
          }
        }
      }
  ...
  non_maxi_suppression(prepare_corner_points, prepare_corner_cdescs);
  if (config_setting_.maximum_corner_num_ < prepare_corner_points->size()) {
    ...
    for (size_t i = 0; i < prepare_corner_points->size(); i++)
      attach_vec.push_back(std::pair<double, int>(prepare_corner_points->points[i].intensity, i));
    std::sort(attach_vec.begin(), attach_vec.end(), attach_greater_sort);
    pcl::PointCloud<pcl::PointXYZINormal>::Ptr tmp_corner_points(
        new pcl::PointCloud<pcl::PointXYZINormal>);
    std::vector<Eigen::Matrix64f> tmp_corner_cdescs;
    for (size_t i = 0; i < config_setting_.maximum_corner_num_ && i < attach_vec.size(); i++) {
      tmp_corner_points->points.push_back(prepare_corner_points->points[attach_vec[i].second]);
      tmp_corner_cdescs.push_back(prepare_corner_cdescs[attach_vec[i].second]);
    }
    *corner_points = *tmp_corner_points;
    corner_cdescs = tmp_corner_cdescs;
  } else {
    *corner_points = *prepare_corner_points;
    corner_cdescs = prepare_corner_cdescs;
  }
}
...
void STDescManager::extract_corner(
    ...
    const std::vector<Eigen::Vector6d>& proj_points,
    pcl::PointCloud<pcl::PointXYZINormal>::Ptr& corner_points,
    std::vector<Eigen::Matrix64f>& corner_cdescs) {
  ...
  std::vector<Eigen::Vector3d> point_list_3d;
  std::vector<Eigen::Vector3f> point_list_rgb;

  for (size_t i = 0; i < proj_points.size(); i++) {
    double x = proj_points[i][0];
    double y = proj_points[i][1];
    double z = proj_points[i][2];
    double r = proj_points[i][3];
    double g = proj_points[i][4];
    double b = proj_points[i][5];
    ...
    double project_z = dis;

    Eigen::Vector3d p_3d(project_x, project_y, project_z);
    Eigen::Vector3f p_rgb(r, g, b);
    point_list_3d.push_back(p_3d);
    point_list_rgb.push_back(p_rgb);
  }
  ...
  if (point_list_3d.size() <= 5) {
    return;
  }

  for (auto pi : point_list_3d) {
    if (pi[0] < min_x) {
      min_x = pi[0];
    }
    if (pi[0] > max_x) {
  ...
  std::vector<Eigen::Matrix<float, 6, 1>> img_container[x_axis_len][y_axis_len];
  ...
  for (int x = 0; x < x_axis_len; x++) {
    for (int y = 0; y < y_axis_len; y++) {
      ...
      std::vector<Eigen::Matrix<float, 6, 1>> single_container;
      img_container[x][y] = single_container;
    }
  }

  for (size_t i = 0; i < point_list_3d.size(); i++) {
    int x_index = (int)((point_list_3d[i][0] - min_x) / resolution);
    int y_index = (int)((point_list_3d[i][1] - min_y) / resolution);
    mean_x_array[x_index][y_index] += point_list_3d[i][0];
    mean_y_array[x_index][y_index] += point_list_3d[i][1];
    img_count_array[x_index][y_index]++;

    Eigen::Matrix<float, 6, 1> p_6d;
    p_6d << point_list_3d[i][0], point_list_3d[i][1], point_list_3d[i][2],
        point_list_rgb[i][0], point_list_rgb[i][1], point_list_rgb[i][2];
    img_container[x_index][y_index].push_back(p_6d);
  }
  ...
  corner_cdescs.reserve(max_gradient_vec.size());
  ...
  for (size_t i = 0; i < max_gradient_vec.size(); i++) {
    ...
    if (is_add) {
      ...
      Eigen::Matrix64f z_rgbn_profile = Eigen::Matrix64f::Zero();
      std::vector<int> z_count_layers(6, 0);

      for (const auto& point : img_container[max_gradient_x_index_vec[i]][max_gradient_y_index_vec[i]]) {
        double z_height = point[2];
        if (z_height >= 0 && z_height < 3.0) {
          int layer_idx = static_cast<int>(z_height / 0.5);
          layer_idx = std::min(5, std::max(0, layer_idx));
          if (point[3] > 0 || point[4] > 0 || point[5] > 0) {
            z_rgbn_profile(layer_idx, 0) += point[3];
            z_rgbn_profile(layer_idx, 1) += point[4];
            z_rgbn_profile(layer_idx, 2) += point[5];
          }
          z_rgbn_profile(layer_idx, 3) += 1.0;
          z_count_layers[layer_idx]++;
        }
      }
      for (int layer = 0; layer < 6; layer++) {
        if (z_count_layers[layer] > 0) {
          z_rgbn_profile(layer, 0) /= z_count_layers[layer];
          z_rgbn_profile(layer, 1) /= z_count_layers[layer];
          z_rgbn_profile(layer, 2) /= z_count_layers[layer];
        }
      }
      corner_cdescs.push_back(z_rgbn_profile);
...
void STDescManager::non_maxi_suppression(
  ...
  for (size_t i = 0; i < corner_points->size(); i++) {
    prepare_key_cloud->push_back(corner_points->points[i]);
    is_add_vec.push_back(true);
  }
  prepare_corner_cdescs = corner_cdescs;
  ...
  for (size_t i = 0; i < is_add_vec.size(); i++) {
    if (is_add_vec[i]) {
      ...
      corner_cdescs.push_back(prepare_corner_cdescs[i]);
    }
  }
}
...
void STDescManager::build_stdesc(
    ...
    const std::vector<Eigen::Matrix64f>& corner_cdescs,
    std::vector<STDesc>& stds_vec) {
  stds_vec.clear();
    ...
    if (kd_tree->nearestKSearch(searchPoint, near_num, pointIdxNKNSearch,
      ...
      for (int m = 1; m < near_num - 1; m++) {
        for (int n = m + 1; n < near_num; n++) {
          ...
          Eigen::Matrix64f corner_cdescs_1 = corner_cdescs[i];
          Eigen::Matrix64f corner_cdescs_2 = corner_cdescs[pointIdxNKNSearch[m]];
          Eigen::Matrix64f corner_cdescs_3 = corner_cdescs[pointIdxNKNSearch[n]];
          ...
          if (iter == feat_map.end()) {
            ...
            single_descriptor.corner_cdescs_A_ = corner_cdescs_A;
            single_descriptor.corner_cdescs_B_ = corner_cdescs_B;
            single_descriptor.corner_cdescs_C_ = corner_cdescs_C;
...
void STDescManager::candidate_descriptor_selector(
    const std::vector<STDesc>& stds_vec,
    std::vector<STDMatchList>& candidate_matcher_vec) {
  ...
  int dis_match_cnt = 0, loc_match_cnt = 0;
#ifdef MP_EN
...
#pragma omp parallel for
  ...
  for (size_t i = 0; i < stds_vec.size(); i++) {
    ...
    STDesc_LOC position;
    double dis_threshold = src_std.side_length_.norm() * config_setting_.side_length_thresh_;
    for (auto voxel_inc : voxel_round) {
            ...
            if ((src_std.frame_id_ - data_base_[position][j].frame_id_) >
              ...
              if (dis < dis_threshold) {
                ...
                double vertex_attach_diff =
                    2.0 * (src_std.vertex_attached_ - data_base_[position][j].vertex_attached_).norm() /
                    (src_std.vertex_attached_ + data_base_[position][j].vertex_attached_).norm();
                bool vertex_attach_pass = vertex_attach_diff < config_setting_.vertex_diff_thresh_;

                float diffA = compute_cdesc_diff(src_std.corner_cdescs_A_, data_base_[position][j].corner_cdescs_A_);
                float diffB = compute_cdesc_diff(src_std.corner_cdescs_B_, data_base_[position][j].corner_cdescs_B_);
                float diffC = compute_cdesc_diff(src_std.corner_cdescs_C_, data_base_[position][j].corner_cdescs_C_);
                bool vertex_cdesc_pass = diffA < config_setting_.cdesc_diff_thresh_ &&
                                         diffB < config_setting_.cdesc_diff_thresh_ &&
                                         diffC < config_setting_.cdesc_diff_thresh_;

                if (vertex_attach_pass &&
                    (!config_setting_.cdesc_verify_en_ || vertex_cdesc_pass)) {
                  loc_match_cnt++;
                  useful_match[i] = true;
  ...
  ROS_INFO("[Candidate Selector] Initial matching stats: rough matches: %d, local matches: %d",
           dis_match_cnt, loc_match_cnt);
  ...
  for (size_t i = 0; i < useful_match.size(); i++) {
    if (useful_match[i]) {
      for (size_t j = 0; j < useful_match_index[i].size(); j++) {
        const auto& pos = useful_match_position[i][j];
        size_t idx = useful_match_index[i][j];
        assert(data_base_.find(pos) != data_base_.end());
        const auto& vec = data_base_[pos];
        assert(idx < vec.size());
        int frame_id = vec[idx].frame_id_;
        match_array[frame_id] += 1;
        index_recorder.push_back(Eigen::Vector2i(i, j));
        match_index_vec.push_back(frame_id);
      }
    }
  }
  ...
  for (int cnt = 0; cnt < config_setting_.candidate_num_; cnt++) {
    ...
    if (max_vote_index >= 0 && max_vote >= 5) {
      ...
      candidate_ids.push_back(max_vote_index);
      added_candidates++;
    } else {
      break;
    }
  ...
  if (!candidate_ids.empty()) {
    ...
    ROS_INFO("[Candidate Selector] Added %d candidates: [%s]", added_candidates, ss.str().c_str());
  } else {
    ROS_INFO("[Candidate Selector] Added 0 candidates");
  }
}

float STDescManager::compute_cdesc_diff(const Eigen::Matrix64f& v1, const Eigen::Matrix64f& v2) {
  // float diff_r = 0, diff_g = 0, diff_b = 0, diff_cnt = 0;
  // float norm_r1 = v1.col(0).norm(), norm_r2 = v2.col(0).norm();
  // float norm_g1 = v1.col(1).norm(), norm_g2 = v2.col(1).norm();
  // float norm_b1 = v1.col(2).norm(), norm_b2 = v2.col(2).norm();
  // float norm_cnt1 = v1.col(3).norm(), norm_cnt2 = v2.col(3).norm();

  // if (norm_r1 > 1e-6 && norm_r2 > 1e-6)
  //   diff_r = 1.f - std::fabs(v1.col(0).dot(v2.col(0))) / (norm_r1 * norm_r2);
  // if (norm_g1 > 1e-6 && norm_g2 > 1e-6)
  //   diff_g = 1.f - std::fabs(v1.col(1).dot(v2.col(1))) / (norm_g1 * norm_g2);
  // if (norm_b1 > 1e-6 && norm_b2 > 1e-6)
  //   diff_b = 1.f - std::fabs(v1.col(2).dot(v2.col(2))) / (norm_b1 * norm_b2);
  // if (norm_cnt1 > 1e-6 && norm_cnt2 > 1e-6)
  //   diff_cnt = 1.f - std::fabs(v1.col(3).dot(v2.col(3))) / (norm_cnt1 * norm_cnt2);

  // return std::fmin((diff_r + diff_g + diff_b) / 3.0f, diff_cnt);

  float r_diff = 0, g_diff = 0, b_diff = 0, cnt_diff = 0;
  float r_norm_sum = (v1.col(0) + v2.col(0)).norm();
  float g_norm_sum = (v1.col(1) + v2.col(1)).norm();
  float b_norm_sum = (v1.col(2) + v2.col(2)).norm();
  float cnt_norm_sum = (v1.col(3) + v2.col(3)).norm();

  if (r_norm_sum > 1e-6)
    r_diff = 2.0f * (v1.col(0) - v2.col(0)).norm() / r_norm_sum;
  if (g_norm_sum > 1e-6)
    g_diff = 2.0f * (v1.col(1) - v2.col(1)).norm() / g_norm_sum;
  if (b_norm_sum > 1e-6)
    b_diff = 2.0f * (v1.col(2) - v2.col(2)).norm() / b_norm_sum;
  if (cnt_norm_sum > 1e-6)
    cnt_diff = 2.0f * (v1.col(3) - v2.col(3)).norm() / cnt_norm_sum;

  int rgb_valid = 0;
  if (r_norm_sum > 1e-6) rgb_valid++;
  if (g_norm_sum > 1e-6) rgb_valid++;
  if (b_norm_sum > 1e-6) rgb_valid++;

  float rgb_avg = rgb_valid > 0 ? (r_diff + g_diff + b_diff) / rgb_valid : 1.0f;
  float cnt_val = (cnt_norm_sum > 1e-6) ? cnt_diff : 1.0f;

  return std::fmin(rgb_avg, cnt_val);
}
...
void STDescManager::candidate_verify(
  ...
  int vote_passed = 0, color_passed = 0, geometry_passed = 0, final_passed = 0;
...
#pragma omp parallel for
  ...
  for (size_t i = 0; i < use_size; i++) {
    ...
    for (size_t j = 0; j < candidate_matcher.match_list_.size(); j++) {
      ...
      bool geodesc_passed = dis_A < dis_threshold && dis_B < dis_threshold && dis_C < dis_threshold;
      // float cos_sim_A = compute_cdesc_diff(verify_pair.first.corner_cdescs_A_, verify_pair.second.corner_cdescs_A_);
      // float cos_sim_B = compute_cdesc_diff(verify_pair.first.corner_cdescs_B_, verify_pair.second.corner_cdescs_B_);
      // float cos_sim_C = compute_cdesc_diff(verify_pair.first.corner_cdescs_C_, verify_pair.second.corner_cdescs_C_);
      // bool cdesc_passed = cos_sim_A > config_setting_.cdesc_diff_thresh_ &&
      //                     cos_sim_B > config_setting_.cdesc_diff_thresh_ &&
      //                     cos_sim_C > config_setting_.cdesc_diff_thresh_;
      // cdesc_passed = !config_setting_.cdesc_verify_en_ || cdesc_passed;
      // if (geodesc_passed && cdesc_passed) vote++;
      if (geodesc_passed) vote++;
    }
    mylock.lock();
    vote_list[i] = vote;
    mylock.unlock();
  }
  int max_vote_index = 0, max_vote = 0;
  ...
  if (max_vote >= 4) {
    ...
    for (size_t j = 0; j < candidate_matcher.match_list_.size(); j++) {
      ...
      double dis_C = (C_transform - verify_pair.second.vertex_C_).norm();
      bool geodesc_passed = dis_A < dis_threshold && dis_B < dis_threshold && dis_C < dis_threshold;
      if (geodesc_passed) geometry_passed++;
      // float cos_sim_A = compute_cdesc_diff(verify_pair.first.corner_cdescs_A_, verify_pair.second.corner_cdescs_A_);
      // float cos_sim_B = compute_cdesc_diff(verify_pair.first.corner_cdescs_B_, verify_pair.second.corner_cdescs_B_);
      // float cos_sim_C = compute_cdesc_diff(verify_pair.first.corner_cdescs_C_, verify_pair.second.corner_cdescs_C_);
      // bool cdesc_passed = cos_sim_A > config_setting_.cdesc_diff_thresh_ &&
    ...
    verify_score = plane_geometric_verify(
        plane_cloud_vec_[current_frame_id_],
        plane_cloud_vec_[candidate_matcher.match_id_.second], relative_pose);
    if (verify_score > config_setting_.planegeo_icp_thresh_) final_passed = 1;
    ROS_INFO("[Verify] Frame %d->%d: stds:%zu, Color:%d, Geometry:%d, Vote:%d, Score:%.3f",
             candidate_matcher.match_id_.first, candidate_matcher.match_id_.second,
             candidate_matcher.match_list_.size(), color_passed, geometry_passed, vote_passed, verify_score);
  }
}
...
void STDescManager::triangle_solver(std::pair<STDesc, STDesc>& std_pair,
  ...
  if (!V.allFinite() || !U.allFinite()) {
    rot = Eigen::Matrix3d::Identity();
    t = -std_pair.first.center_ + std_pair.second.center_;
    return;
  }
...
double STDescManager::plane_geometric_verify(
  ...
  double planegeo_normal_thresh = config_setting_.planegeo_normal_thresh_;
  double dis_threshold = config_setting_.planegeo_dis_thresh_;
  for (size_t i = 0; i < source_cloud->size(); i++) {
    ...
    if (kd_tree->nearestKSearch(use_search_point, K, pointIdxNKNSearch,
      ...
      for (size_t j = 0; j < K; j++) {
        ...
        if ((normal_inc.norm() < planegeo_normal_thresh ||
             normal_add.norm() < planegeo_normal_thresh) &&
...
void STDescManager::PlaneGeomrtricIcp(
  ...
  for (size_t i = 0; i < source_cloud->size(); i++) {
    ...
    if (kd_tree->nearestKSearch(use_search_point, 1, pointIdxNKNSearch,
      ...
      double point_to_plane = fabs(tni.transpose() * (pi - tpi));
      if ((normal_inc.norm() < config_setting_.planegeo_normal_thresh_ ||
           normal_add.norm() < config_setting_.planegeo_normal_thresh_) &&
          point_to_plane < config_setting_.planegeo_dis_thresh_ &&
