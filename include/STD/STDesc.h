...
namespace Eigen {
typedef Eigen::Matrix<float, 6, 4> Matrix64f;
...
struct ConfigSetting {
  ...
  int skip_near_num_ = 10;
  int candidate_num_ = 50;
  bool cdesc_verify_en_ = false;
  double cdesc_diff_thresh_ = 0.5;
  double side_length_thresh_ = 0.03;
  double vertex_diff_thresh_ = 0.7;
  double planegeo_normal_thresh_ = 0.1;
  double planegeo_dis_thresh_ = 0.3;
  double planegeo_icp_thresh_ = 0.5;
};
...
struct STDesc {
  ...
  Eigen::Matrix64f corner_cdescs_A_;
  Eigen::Matrix64f corner_cdescs_B_;
  Eigen::Matrix64f corner_cdescs_C_;
};
...
void publish_std_descs(const std::vector<STDesc>& std_pairs,
                       const ros::Publisher& std_publisher);
...
class STDescManager {
 public:
  ...
  void GenerateSTDescs(PointCloud::Ptr& input_cloud,
  ...
  void AddSTDescs(const std::vector<STDesc>& stds_vec, const Eigen::Affine3d& pose = Eigen::Affine3d::Identity());
  ...
  std::vector<PointCloud::Ptr> key_cloud_vec_;
  ...
  std::vector<Eigen::Affine3d> key_pose_vec_;

 private:
  void init_voxel_map(const PointCloud::Ptr& input_cloud,
  ...
  void build_stdesc(const pcl::PointCloud<pcl::PointXYZINormal>::Ptr& corner_points,
                    ...
                    std::vector<STDesc>& stds_vec);
  void candidate_descriptor_selector(const std::vector<STDesc>& stds_vec,
                                     std::vector<STDMatchList>& candidate_matcher_vec);
  float compute_cdesc_diff(const Eigen::Matrix64f& v1, const Eigen::Matrix64f& v2);
  ...
  void candidate_distance_selector(std::vector<STDMatchList>& candidate_matcher_vec);
};
